...
classifierName={'KNN Classifier k-3','KNN Classifier k-5','KNN Classifier k-7','Decision Tree','SVM','Linear SVM Model'};
...
for i=1:foldSize
    ...
    ResultsSVM2 = table(classifierName(6), foldSize, accuracySVM2,sensitivitySVM2,specificitySVM2);
...
ResultsAlcohol9=table(classifierName(6), foldSize, mean(accuracySVM2),mean(sensitivitySVM2),mean(specificitySVM2));
...
%ResultsAlcohol.Properties.VariableNames ={'Classifier Name','Fold Size','Accuracy','Sensitivity','specificity'};
...
title({'Accuracy for all Classification Models ';'\color{red}\fontsize{16}'});
ylabel('Accuracy');
...
filename='ResultsAlcohol';
writetable( ResultsAlcohol, [ filename '.csv'], 'Delimiter', ',', 'WriteRowNames', true);
